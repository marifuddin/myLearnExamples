...
public class VmwareEducationPage extends myutils{
	...
	public void sendTextToSearchContent(String value)
	{
		...
		String boxXpath = "//input[@name='keyword']";
		String selectXpath = "//input[@type='submit']";
		
	
		if(waitOnXpath(boxXpath, 10))
		{
			cleartextField(boxXpath);
			setTextField(boxXpath, value);
			sendReport("Search the class with ID " + value , "Successfully Enterted the value in search box", "Pass");
	...
	{
		String TryPowerClasses="//span[text()='Try power search']/..";
		//String catalogXpath = "//select[@name='category']";
		if(waitOnXpath(TryPowerClasses, timeOutSecs()));
		{
			clickOnElement(TryPowerClasses);
			clickOnElement("//span[text()='Classes']/..");
			...
			clickOnElement("//input[@value='Find Class']");
			sendReport("Clicking on the Search Button ", "Successfully Clicked on the Search Button", "Pass");
		}
	}
	public void changeCountyPreference(String country) throws IOException
	{
		String cross = "//strong[text()='Country:']/preceding-sibling::img";
		String changeCounty = "//a[text()='change your preferences']";
		
		String setpreference = "//input[@name='save']";
		String courseLink="http://mylearn.vmware.com/portals/www/search/results.cfm?ui=www_edu&menu=search-results&searchtype=simple&category=schedule&id_subject=46424&filters=countryID&countryID=14&autofilters=countryID";
		getRespectiveCourseUrl(courseLink);
		try {
			if(waitOnXpath(cross, timeOutSecs()))
			{
				clickOnElement(cross);
				waitOnXpath("//a[text()='"+country+"']", 10);
				clickOnElement("//a[text()='"+country+"']");
	...
	public void searchCourse()
	{
		...
		if(waitOnXpath(TryPowerClasses, timeOutSecs()))
		{
			clickOnElement(TryPowerClasses);
			clickOnElement("//span[text()='Classes']/..");
			sendReport("Selecting the Value from Drop Down ", "Successfully Selected courses from Drop Down ", "Pass");
			clickOnElement("//input[@value='Find Class']");
			sendReport("Clicking on the Search Button ", "Successfully Clicked on the Search Button", "Pass");
		}
